...
# A bid triggers the stop price to be higher than the token price, thus ending
# the auction instantly.  Based on test_dao_dutch_auction_stop_price
class TestContract(AbstractTestContract):
    ...
    def test(self):
        ...
        self.assertEqual(self.dao_auction.calcTokenPrice(), 20000 * 10**18 / (days_later + 1))
        self.assertGreater(self.dao_auction.calcTokenPrice(), self.dao_auction.calcStopPrice())
        ...
        self.assertRaises(TransactionFailed, self.dao_auction.bid, sender=keys[bidder_2], value=1)
        ...
        self.assertEqual(self.dao_token.balanceOf(accounts[bidder_1]),
                         self.dao_token.balanceOf(accounts[bidder_2]))
        self.assertEqual(
